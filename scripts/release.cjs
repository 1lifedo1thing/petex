...
const expected=[`Pedex-${version}-mac-arm64.dmg`,`Pedex-${version}-mac-arm64.zip`,`Pedex-${version}-mac-x64.dmg`,`Pedex-${version}-mac-x64.zip`,`Pedex-${version}-win-x64.exe`];
...
if(existing.status!==0)gh(['release','create',tag,'--draft','--target',process.env.RELEASE_COMMIT,'--title',`Pedex ${version}`,'--generate-notes']);
